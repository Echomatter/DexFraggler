...
export async function GET(req:Request){try{
 ...
 if(url.searchParams.has('export')){
  ...
  return json({format:'dexfraggler-table',version:4,targetVersion:TARGET_VERSION,metricVersion:METRIC_VERSION,config,targets,model:MODEL,cells});
 }
...
export async function POST(req:Request){try{
 ...
 if(x.action==='claim_row'){
  if(x.model!==MODEL||x.targetVersion!==TARGET_VERSION||x.metricVersion!==METRIC_VERSION)return json({error:'Restart DexFraggler to load the current search engine.'},409);
