...
export type Target={kind:string;weights?:number[];samples?:number[];sampleRate?:number;fundamentalHz?:number;harmonics?:{sin:number[];cos:number[]};id?:string;source?:Record<string,unknown>;preprocessVersion?:string};
export type Anchor={slot:number;shape:string};
export type Config={allowDetune:boolean;anchors:Anchor[];targetSet?:Target[];source?:Record<string,unknown>};
...
export type Data={scan:{id:string;name:string};pendingSeeds:number;config:Config;revision:number;generation:number;running:boolean;activeId:number|null;activeIds?:number[];cells:Summary[];workers:{id:string;last_seen:number;engine:string;status?:{cellsPerMinute?:number;scoreCacheEntries?:number;priority?:string;phase?:string;localPaused?:boolean}}[]};
