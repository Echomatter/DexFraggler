...
export type Target={kind:string;sin:number[];cos:number[]};
export type Anchor={slot:number;shape:string;name?:string;target?:Target};
export type Config={harmonics:number;allowDetune:boolean;anchors:Anchor[]};
export type Summary={id:number;current:boolean;visits:number;evaluations:number;model_score:string|null;native_score:string|null;revision:number;updated_at:number;last_error?:string|null};
export type Measurement={note:number;score:number;error:number;wave:number[];target:number[]};
...
export function useTable(){
 const [selected,setSelected]=useState(0),[data,setData]=useState<Data|null>(null),[cell,setCell]=useState<Cell|null>(null),[error,setError]=useState(''),[notice,setNotice]=useState(''),[busy,setBusy]=useState(false),selection=useRef(0);
 ...
 const load=useCallback(async()=>{const r=await fetch('/api/table',{cache:'no-store'}),d=await r.json() as Data&{error:string};if(!r.ok)throw Error(d.error);setData(prev=>prev&&prev.revision>d.revision?prev:d);return d},[]);
 ...
 useEffect(()=>{let alive=true;setCell(prev=>prev?.id===selected?prev:null);fetch('/api/table?cell='+selected,{cache:'no-store'}).then(async r=>{const d=await r.json() as {cell:Cell|null;error:string};if(!r.ok)throw Error(d.error);if(alive)setCell(d.cell)}).catch(e=>{if(alive)setError(e.message)});return()=>{alive=false}},[selected,revision,data?.generation]);
 const config:Config=data?.config??DEFAULT_CONFIG,targets=useMemo(()=>columnTargets(config) as Target[],[config]);
 return {selected,select,selection,data,cell,config,targets,error,setError,notice,setNotice,busy,setBusy,load,action,perform};
}
