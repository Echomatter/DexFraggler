...
- Reads `.runtime/runner-status.json`: `{pid,nativePid,root,processStartTime,nativeProcessStartTime,running,localPaused,priority,phase,updatedAt,evaluations,scoreCacheEntries}`. Times can be Unix milliseconds or ISO strings. Node start time may be computed as `Date.now()-process.uptime()*1000`.
