...
- Download uses authenticated GET `/api/table?export=1`. It requires `format: "dexfraggler-table"`, `version: 4`, `targetVersion: "ideal-waveform-v1"`, 0-1,024 cells and a 32 MB limit; a new table with no results can be exported. Config contains exactly `allowDetune` and 1-32 sorted anchors `{slot,shape}` with unique slots 0-31 and shapes sine, triangle, square or saw. Exactly 32 targets must each contain `{kind:"ideal-waveform-v1",weights:[sine,triangle,square,saw]}` with four finite nonnegative weights summing to 1 within 1e-12. The validated response atomically replaces the chosen file and preserves all returned results and measurement metadata. Other export versions, custom/Fourier targets and harmonic-count configuration are rejected before saving.
