...
An FFT evaluates an initial phase grid. Adaptive subdivision uses a global second-derivative bound: within an interval of width \(d\), correlation cannot exceed the larger endpoint value plus \(Md^2/8\). Only intervals that could improve the best score are subdivided. The stopping tolerance is \(10^{-10}\) in correlation; Newton refinement then improves phase precision. This bound applies in exact arithmetic, with a separate floating-point allowance in the implementation.
