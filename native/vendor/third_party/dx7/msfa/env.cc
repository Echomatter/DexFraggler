...
// WaveFinder portability: no application/JUCE dependency is needed here.
//using namespace std;
...
void Env::update(const int r[4], const int l[4], int ol, int rate_scaling) {
    ...
    outlevel_ = ol;
    rate_scaling_ = rate_scaling;
    // WaveFinder: retain attack/decay/release progression on live edits.
    // A held sustain is represented by ix_ == 3; revisit level 3 in that case.
    if (initialised_)
        advance(down_ && ix_ == 3 ? 2 : ix_);
}
