...
int32_t Freqlut::lookup(int32_t logfreq) {
  ...
  // WaveFinder: legal extreme pitch EG / bend / coarse combinations can exceed
  // the original LUT octave range. Avoid undefined negative or >=32 shifts;
  // phase deltas are periodic modulo 2^24, matching oscillator phase wrapping.
  const int shift = MAX_LOGFREQ_INT - hibits;
  if (shift >= 32) return 0;
  if (shift >= 0) return (static_cast<uint32_t>(y) >> shift) & 0xffffff;
  if (shift <= -32) return 0;
  return (static_cast<uint32_t>(y) << -shift) & 0xffffff;
}
