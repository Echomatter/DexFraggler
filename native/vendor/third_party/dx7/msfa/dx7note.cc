...
int32_t Dx7Note::osc_freq(int midinote, int mode, int coarse, int fine, int detune, int channel) {
    ...
    int32_t logfreq;
    if (mode == 0) {
        // WaveFinder: deterministic standard tuning; optional MTS-ESP is omitted.
...
Dx7Note::Dx7Note(std::shared_ptr<TuningState> ts)
: tuning_state_(ts) {
    initialised_ = false;
    fb_buf_[0] = fb_buf_[1] = 0;
...
void Dx7Note::oscSync() {
    // WaveFinder: key-synced note-on also starts a deterministic feedback state.
    fb_buf_[0] = fb_buf_[1] = 0;
    for (int i=0;i<6;i++) {
