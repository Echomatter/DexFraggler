...
for(let i=0;i<cells.length;i+=64){const r=await fetch(config.url+'/api/table',{method:'POST',headers,body:JSON.stringify({action:'import',cells:cells.slice(i,i+64).map(c=>({algorithm:c.algorithm,slot:c.slot,patch:c.patch,provenance:c.provenance??{source:'WaveLab',score:c.score,model:data.model}}))})});if(!r.ok)throw Error(`${r.status} ${await r.text()}`);console.log(`Imported ${Math.min(i+64,cells.length)} / ${cells.length}`)}
